...
fn build_format_current_file_template_command(format_command: &str) -> String {
    ...
    let shell_command = "bash -c 'cwd=\"${CURSOR_PROJECT_DIR:-$CLAUDE_PROJECT_DIR}\"; if [ -z \"$cwd\" ] && [ ! -t 0 ]; then json=$(cat); cwd=$(echo \"$json\" | grep -oE \"\\\"workspacePaths\\\"\\\\s*:\\\\s*\\\\[\\\\s*\\\"[^\\\"]+\\\"\" | sed \"s/.*\\\"\\\\(.*\\\\)\\\".*/\\\\1/\"); if [ -z \"$cwd\" ]; then cwd=$(echo \"$json\" | grep -oE \"\\\"workspace_roots\\\"\\\\s*:\\\\s*\\\\[\\\\s*\\\"[^\\\"]+\\\"\" | sed \"s/.*\\\"\\\\(.*\\\\)\\\".*/\\\\1/\"); fi; if [ -z \"$cwd\" ]; then cwd=$(echo \"$json\" | grep -oE \"\\\"cwd\\\"\\\\s*:\\\\s*\\\"[^\\\"]+\\\"\" | sed \"s/.*\\\"\\\\(.*\\\\)\\\".*/\\\\1/\"); fi; fi; if [ -z \"$cwd\" ]; then cwd=\".\"; fi; cd \"$cwd\" || exit 1; template=$(echo \"ENCODED_CMD\" | xxd -r -p); files=$(git status --porcelain | awk \"{print \\$2}\"); if [ -n \"$files\" ]; then for f in $files; do if [ -f \"$f\" ]; then if [[ \"$template\" == *\"{{current_file}}\"* ]]; then cmd=\"${template//\\{\\{current_file\\}\\}/$f}\"; else cmd=\"$template $f\"; fi; eval \"$cmd\"; fi; done; fi; echo \"{\\\"continue\\\": true}\"'";
