...
describe("AccountSwitcher Component", () => {
  const mockMessageApi = {
    ...
    info: vi.fn(),
    loading: vi.fn(),
  } as unknown as ReturnType<typeof import("@xinghunm/compass-ui").Message.useMessage>[0];
  ...
  test("renders empty state and allows saving new account", async () => {
    ...
    await userEvent.click(saveBtn);
    expect(mockMessageApi.warning).toHaveBeenCalledWith("账号名称仅允许字母、数字、下划线、短横线、@ 及 . (且不能包含连续点)");
    ...
    await userEvent.click(saveBtn);
    expect(api.saveCurrentEditorAccount).toHaveBeenCalledWith("codex", "fengyueran@gmail.com");
  ...
  test("lists accounts, allows switching and deletion", async () => {
    ...
    const activeSwitchBtn = switchButtons.find((btn) => !btn.hasAttribute("disabled"));
    ...
    expect(mockMessageApi.success).toHaveBeenCalledWith("已删除账号备份: personal");
  });
});
