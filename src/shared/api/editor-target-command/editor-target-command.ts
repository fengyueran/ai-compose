...
export async function applyPromptToEditorTarget(
  ...
  return invoke<ApplyPromptResult>('apply_prompt_to_editor_target', {
    payload,
  });
}
...
> {
  return invoke<Record<EditorId, EditorTargetState>>(
...
export async function loadEditorHooksStates(): Promise<HooksConfigState> {
...
export async function applySkillsToEditorTarget(
  ...
  return invoke<ApplySkillsResult>('apply_skills_to_editor_target', {
    payload,
  });
}
...
export async function addSkillsRepository(
  repo: string,
): Promise<AddSkillsRepositoryResult> {
