...
import { type McpServer, presetMcpServers } from '../mcp-servers';
  ...
  try {
    const data =
      typeof window !== 'undefined'
        ? localStorage.getItem('ai-compose:custom-skill-sources')
        : null;
...
const loadCustomServersFromStorage = (): McpServer[] => {
  try {
    const data =
      typeof window !== 'undefined'
        ? localStorage.getItem('ai-compose:custom-mcp-servers')
        : null;
    ...
    return list
      ...
      .map((item) => {
        const id = item.id.startsWith('custom-')
          ? item.id
          : `custom-${item.id}`;
        return {
          ...item,
...
type AiComposeState = {
  ...
  updateHook: (
    hookId: string,
    update: Partial<Omit<HookDefinition, 'id'>>,
  ) => void;
  updateHookCommand: (
    hookId: string,
    commandId: string,
...
const initialMcpEnabledServerIdsByEditor = {
  antigravity: presetMcpServers
    .filter((server) => server.enabled)
    .map((server) => server.id),
  codex: presetMcpServers
    .filter((server) => server.enabled)
    .map((server) => server.id),
...
const buildEnabledServerIdsByEditor = (
  ...
  const nextEnabledIds: Record<EditorId, string[]> = {
    antigravity: resolveMcpServerIdsFromManaged(
      nextMcpStates.antigravity.managedMcpServers,
      updatedServers,
    ),
    codex: resolveMcpServerIdsFromManaged(
      nextMcpStates.codex.managedMcpServers,
      updatedServers,
    ),
  ...
  (Object.keys(nextEnabledIds) as EditorId[]).forEach((editorId) => {
    if (
      nextEnabledIds[editorId].length === 0 &&
      !nextMcpStates[editorId].managedMcpServers
    ) {
...
export const useAiComposeStore = create<AiComposeState>((set, get) => ({
  ...
  hydratePromptEditorStates: (editorStates) => {
    ...
    set((state) => ({
      ...
      editorStates:
        state.activeDomain === 'Prompt' ? nextPromptStates : state.editorStates,
  ...
  hydrateMcpEditorStates: (editorStates) => {
    ...
    const checkEditorEnabled = (targetState: typeof editorStates.codex) => {
      ...
      return Object.values(managedMcp).some((val) => {
        return (
          val && typeof val === 'object' && ('command' in val || 'url' in val)
        );
      });
    };
    ...
    const nextSelectedMcpServerId = mcpServers.some(
    ...
    set((state) => ({
      ...
      editorStates:
        state.activeDomain === 'MCP' ? nextMcpStates : state.editorStates,
  ...
  hydrateHooksEditorStates: (editorStates) => {
    ...
    set((state) => ({
      ...
      editorStates:
        state.activeDomain === 'Hooks' ? nextHooksSummary : state.editorStates,
      isHydratingEditorStates: false,
    }));
  },
  ...
  hydrateSkillsEditorStates: (editorStates) => {
    ...
    set((state) => ({
      ...
      editorStates:
        state.activeDomain === 'Skills' ? nextSkillsStates : state.editorStates,
      isHydratingEditorStates: false,
    }));
  },
  ...
  toggleFragment: (fragmentId) => {
    ...
    if (isEnabled) {
      set({
        enabledFragmentIds: enabledFragmentIds.filter(
          (id) => id !== fragmentId,
        ),
      });
      return;
  ...
  addMcpServer: (server) => {
    ...
    set((state) => {
      ...
      const currentEnabledIds =
        state.mcpEnabledServerIdsByEditor[activeEditorId] ?? [];
  ...
  deleteMcpServer: (id) => {
    ...
    set({
      mcpServers: nextServers,
      mcpEnabledServerIdsByEditor: {
        antigravity: get().mcpEnabledServerIdsByEditor.antigravity.filter(
          (serverId) => serverId !== id,
        ),
        codex: get().mcpEnabledServerIdsByEditor.codex.filter(
          (serverId) => serverId !== id,
        ),
  ...
  setSkillsList: (physicalSkills) => {
    ...
    const mappedPhysical = physicalSkills.map((ps) => {
      const preset = BUILTIN_SKILLS_PRESET.find((p) =>
        isPresetSkillMatch(ps.id, p.id),
      );
      return {
        ...ps,
    ...
    const missingBuiltinSkills = BUILTIN_SKILLS_PRESET.filter(
      (preset) =>
        !mappedPhysical.some((skill) =>
          isPresetSkillMatch(skill.id, preset.id),
        ),
    ).map((preset) => ({
      id: preset.id,
      name: preset.name,
    ...
    const nextSkills = [...mappedPhysical, ...missingBuiltinSkills].sort(
      (a, b) => {
    ...
    if (
      nextSkills.length > 0 &&
      (!selectedSkillId || !nextSkills.some((s) => s.id === selectedSkillId))
    ) {
...
const BUILTIN_SKILLS_PRESET = [
  ...
  },
  {
    ...
    description:
      '指导前端设计实现，使用优质设计系统、TailwindCSS 和组件库，保证卓越视觉和优秀体验',
  ...
  },
  {
    ...
    name: 'brainstorming',
    description:
      '在开始任何功能、组件或行为的开发/修改前，用于理清用户意图和产品设计的头脑风暴指南',
  ...
  },
  {
    ...
    description:
      '当遇到两个或更多无顺序依赖、无共享状态的独立任务时，进行并行任务调度的策略',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '开发完成且测试通过后，指导如何整合代码、提交 PR 或清理临时环境的完成准则',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '接收代码评审意见时的严谨反馈态度指南，强调技术论证和科学验证，拒绝盲从修改',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '在完成任务或主要功能、并准备合并代码前，请求代码评审并验证功能是否完全合规的标准',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '在定位任何 bug、测试失败或意外表现时必须严格执行的系统性排查与调试逻辑',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '遵循先写失败测试、再写最小实现、最后整理重构 (Red-Green-Refactor) 的开发工作流',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '指导如何利用 Git Worktree 隔离开发环境与主工作区，提高分支切换的效率和安全',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '每次开始对话时的第一步，用以确立如何寻找和正确使用已有技能的方法论',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    description:
      '完成任务声称通过前必须履行的验证核对，坚持数据和输出结果高于口头断言',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    name: 'writing-plans',
    description:
      '在有需求、 spec 或多步变更任务时，必须先编写并确认实现方案的计划准则',
    repoSource: 'obra/superpowers',
    content:
  ...
  },
  {
    ...
    name: 'writing-skills',
    description:
      '用于规范和指导在编写、测试并发布新技能或修改现有技能时的质量和结构要求',
    repoSource: 'obra/superpowers',
    content:
