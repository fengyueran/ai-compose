...
export function McpPanel({ messageApi }: McpPanelProps) {
  ...
  const [pendingMcpToggleKey, setPendingMcpToggleKey] = useState<string | null>(
    null,
  );
  ...
  const buildMcpPayload = (
    ...
    const enabledServers = sourceServers.filter((server) =>
      enabledIds.includes(server.id),
    );
  ...
  const applyMcpSelectionToEditor = async (
    ...
    const { enabledServers, payloadData } = buildMcpPayload(
      enabledIds,
      sourceServers,
    );
  ...
  const handleMcpEditorToggle = async (
    ...
    try {
      ...
      messageApi.error(
        `更新 ${editorMeta[editorId].title} 的 MCP 配置失败: ${errMsg}`,
      );
    } finally {
  ...
  const handleSaveMcp = async () => {
    ...
    if (existsName) {
      messageApi.error(
        `已存在名为 "${formName.trim()}" 的 MCP 服务，请使用其他名称`,
      );
      return;
    }
    ...
    if (selectedMcpServerId === '__new__') {
      ...
      if (isTauriRuntime()) {
        ...
        const currentEnabledIds =
          latestStore.mcpEnabledServerIdsByEditor[activeEditorId] ?? [];
        ...
        try {
          await applyMcpSelectionToEditor(
            activeEditorId,
            enabledIdsForEditor,
            latestServers,
          );
      ...
      const enabledEditorIds = editorIds.filter((editorId) =>
        (latestStore.mcpEnabledServerIdsByEditor[editorId] ?? []).includes(
      ...
      const syncResults = await Promise.allSettled(
        enabledEditorIds.map(async (editorId) => {
          const enabledIdsForEditor =
            latestStore.mcpEnabledServerIdsByEditor[editorId] ?? [];
          await applyMcpSelectionToEditor(
            editorId,
            enabledIdsForEditor,
            latestServers,
  ...
  const handleDeleteMcp = async (id: string) => {
    ...
    try {
      const nextEnabledCount = getEnabledMcpIdsForEditor(activeEditorId).filter(
        (serverId) => serverId !== id,
      ).length;

      if (isTauriRuntime()) {
        const { payloadData } = buildMcpPayload(
          getEnabledMcpIdsForEditor(activeEditorId).filter(
            (serverId) => serverId !== id,
          ),
        );
      ...
      messageApi.success({
        content: '删除 MCP 服务成功并已自动同步物理文件！',
        key: 'delete-mcp',
      });
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : String(err);
      messageApi.error({
        content: `删除成功，但物理文件同步失败: ${errMsg}`,
        key: 'delete-mcp',
      });
    }
  };
      ...
      <main className="workbench">
                  ...
                  >
                    <div className="fragment-list__item-main">
                      ...
                      >
                        ...
                        <span
                          className={`mcp-source-badge mcp-source-badge--${server.source}`}
                        >
        ...
        >
              ...
              <div className="mcp-detail-switches">
                ...
                <div className="editor-icon-toggle-group">
                  {editorIds.map((editorId) => {
                    const isEnabled = isMcpEnabledForEditor(
                      editorId,
                      selectedMcpServer.id,
                    );
                    ...
                    return (
                      ...
                      >
                        <button
                          ...
                          disabled={
                            pendingMcpToggleKey ===
                            `${editorId}:${selectedMcpServer.id}`
                          }
                          onClick={() => {
                            void handleMcpEditorToggle(
                              editorId,
                              selectedMcpServer.id,
                            );
                          }}
          ...
          <div className="fragment-detail__body" style={{ overflowY: 'auto' }}>
            ...
            >
              <div
                className="mcp-form__field"
                style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}
              >
                <label
                  className="mcp-form__label"
                  style={{
                    fontSize: '12px',
                    color: 'var(--text-faint)',
                    fontWeight: 500,
          ...
          >
            {previewEnabledMcp.length === 0 ? (
              <p className="preview-card__empty">
                {editorMeta[previewEditorId].title} 当前还没有启用任何 MCP
                服务。请先在中间工作区开启后再查看。
              </p>
            ) : (
              <pre
