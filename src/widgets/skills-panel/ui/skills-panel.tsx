...
export function SkillsPanel({ messageApi }: SkillsPanelProps) {
  ...
  const isSkillsLoading =
    isHydratingEditorStates || isAddingSkillsRepo || isRemovingSkill;
  ...
  const selectedSource = useMemo(() => {
    return (
      skillSources.find((s) => s.id === selectedSkillSourceId) ||
      skillSources[0]
    );
        ...
        selectedSource.type === 'all'
          ...
          : selectedSource.type === 'preset'
            ...
            : selectedSource.type === 'repo'
              ? isSkillFromRepoSource(skill, selectedSource.value)
              : !!skill.path &&
                skill.path
                  .toLowerCase()
                  .startsWith(selectedSource.value.toLowerCase());

      const isLinked = activeEnabledSkillIds.includes(skill.id);
      const canShowUnlinkedSourceSkill =
        selectedSource.type === 'repo' || selectedSource.type === 'local';
      if (
        !skill.isBuiltin &&
        !isLinked &&
        !(canShowUnlinkedSourceSkill && matchesSelectedSource)
      ) {
        return false;
      }
  ...
  const handleSkillEditorToggle = async (
    ...
    if (isLinked) {
      ...
      try {
        ...
        messageApi.success(
          `已取消 ${skill.name} 在 ${editorMeta[editorId].title} 中的链接。`,
        );
      } catch (err) {
    ...
    try {
      ...
      await refreshCurrentEditorSkills(activeEditorId);
      messageApi.success(
        `已将 ${skill.name} 链接到 ${editorMeta[editorId].title}。`,
      );
    } catch (err) {
      const errMsg = err instanceof Error ? err.message : String(err);
      ...
      >
        <div className="skills-list-row__select">
          ...
          <div className="skills-list-row__content">
            ...
            <span
              className="skills-list-row__description"
        ...
        >
          ...
          <div className="skills-manager__body">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="skills-source-item__info">
                        <span className="skills-source-item__name">
                          {source.name}
                        </span>
                        {source.value && (
                          <span
                            className="skills-source-item__value"
                            title={source.value}
                          >
                            {source.value}
                          </span>
            ...
            <div className="skills-list-pane">
              <div
                className="skills-list-pane__header"
                style={{
                  display: 'flex',
                  justifyContent: 'space-between',
                  alignItems: 'center',
              ...
              >
                <div
                  className="skills-list-pane__header-info"
                  style={{
                    display: 'flex',
                ...
                >
                  <span
                    className="skills-list-pane__header-title"
                  ...
                  {selectedSource.type !== 'preset' && (
                    <span
                      className="skills-list-pane__header-subtitle"
                      style={{
                        fontSize: '0.8rem',
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        await refreshCurrentEditorSkills(activeEditorId);
                        messageApi.success(
                          `同步 Git 仓库 ${selectedSource.value} 成功！`,
                        );
                      } catch (err) {
                        const errMsg =
              ...
              <div
                className="skills-list-pane__summary"
                style={{
                  padding: '10px 18px',
              ...
              <div
                className="skills-manager__controls"
                style={{
                  padding: '10px 18px',
                  borderBottom: '1px solid var(--panel-border)',
                  gap: '10px',
              ...
              <div
                className={`skills-list-rows${isSkillsLoading ? ' skills-list-rows--loading' : ''}`}
                style={{ padding: '18px' }}
              >
                {isSkillsLoading && skills.length === 0 ? (
                  ...
                  (() => {
                    ...
                    const presetSkills = filteredSkills.filter(
                      (s) => s.isBuiltin,
                    );
                    ...
                    const repoSources = skillSources.filter(
                      (src) => src.type === 'repo',
                    );
                    repoSources.forEach((src) => {
                      const repoSkills = filteredSkills.filter(
                        (s) =>
                          !s.isBuiltin && isSkillFromRepoSource(s, src.value),
                      );
                      if (repoSkills.length > 0) {
                        groups.push({
                          label: src.name || src.value,
                          skills: repoSkills,
                        });
                      }
                    });

                    const localSources = skillSources.filter(
                      (src) => src.type === 'local',
                    );
                    localSources.forEach((src) => {
                      const localSkills = filteredSkills.filter(
                        (s) =>
                          !s.isBuiltin &&
                          !repoSources.some((r) =>
                            isSkillFromRepoSource(s, r.value),
                          ) &&
                          s.path
                            ?.toLowerCase()
                            .startsWith(src.value.toLowerCase()),
                      );
                        ...
                        {groups.map((group) => (
                          <div key={group.label} className="skills-list-group">
                            <div className="skills-list-group__header">
                              <span className="skills-list-group__title">
                                {group.label}
                              </span>
                              <span className="skills-list-group__count">
